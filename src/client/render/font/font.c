...
void 	loadFont(void)
{
	...
	g_textureID = glhLoadTexture("./font/font.png");
}
...
void 	setFontModelText(t_font_model *model, char const *text)
{
	...
	generateFontBuffer(text, model->step);
	...
	glBindVertexArray(model->vaoID);
	{
		...
		glhVBOData(GL_ARRAY_BUFFER, NULL, 0, GL_STATIC_DRAW);
...
/** generate a model that will be later render with renderFont(...) function */
